...
function is_macro_call(ast: Mal, env: Env): boolean {

	let is_macro = false;
	is_macro = ast.tipo === Types.LIST;
	const mal_list = ast.value as Mal[];
	is_macro &&= mal_list.length > 0;
	is_macro &&= mal_list[0].tipo === Types.SYMBOL;
	const lookedup = is_macro && env.find(mal_list[0].value as string);
	is_macro = !!lookedup && !!(lookedup.value as Fn).is_macro;

	return is_macro;
}

function macroexpand(ast: Mal, env: Env): Mal {
	while(is_macro_call(ast, env)) {
		const mal_list = ast.value as Mal[];
		const mal_function = env.get_data(mal_list[0].value as string) as {value: Fn; tipo: TType};
		ast = mal_function.value.fn.apply(null, mal_list.slice(1));
	}
	return ast;
}
	...
	while (true) {
		...
		switch(first.value) {
			case 'def!':
				...
				return result;
			case 'defmacro!':
				const evaled = EVAL(third, env) as {value: Fn; tipo: TType};
				evaled.value.is_macro = true;
				env.set_data(second.value as string, evaled);
				return evaled;
			case 'let*':
			...
			case 'try*':
				try {
					...
					if (!third)
						throw e;
					const catch_list = third.value as Mal[];
					const catch_env = Envir(env);
					catch_env.set_data(catch_list[1].value as string, {value: e.message as string, tipo: Types.STRING});
					input = catch_list[2];
					env = catch_env;
				}
				break;
